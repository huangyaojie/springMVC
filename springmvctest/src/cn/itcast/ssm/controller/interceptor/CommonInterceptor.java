...
import cn.itcast.ssm.utils.RequestUtil;
import cn.itcast.ssm.utils.UserCookieUtil;

/**
 * [请求拦截器]。<P>
 * [拦截用户请求，验证合法后，转发到相应的URL中。验证不合法，返回登录页面。]
 *
 */
public class CommonInterceptor extends HandlerInterceptorAdapter{
  ...
  */
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        // 判断用户是否登录，一般是都没登录的,
        if(user == null){
            logger.info("user 是null,判断用户为非登录状态，===============");
        	// 取cookie值，这里还有其他网站的
        	...
        	if(cookies!=null && cookies.length>0){
	  			  ...
	  			  if(cookieValue==null){
	  				  ...
	  				  //此处通过员工姓名去数据库中进行检查找 了；
	  				 User temp = userService.findUserById(userId);
	  				  // 如果user返回不为空,就取出密码,使用用户id(userId)+用户名(userName)+密码+有效时间+ webSiteKey进行MD5加密。与前面设置的进行比较，看是否是同一个用户
	  				  if(temp!=null){
	  					  String md5ValueInCookie = cookieValues[2];
	  			       	String md5ValueFromUser = MD5.md5crypt(temp.getUserId() + ":" + temp.getUserName()+":" + temp.getUserPsd()+ ":" + validTimeInCookie + ":" + Const.COOKIEDOMAINNAME);
  		    	  ...
  		    	  logger.info("=user补为null,判断用户为登录===============");
  		    	 //cookie中没有需要的值，此时判断是放行，还是还是去登录页面
  		    	if (url.contains("login")){
				    return true;
			    }
  		    	request.getRequestDispatcher("/WEB-INF/jsp/welcome.jsp").forward(request, response);
